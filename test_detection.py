...
def get_rgbchannel_lines(r_edges_erode, g_edges_erode, b_edges_erode, columns):
    ...
    min_long = int(columns*0.3)
    min_gap = int(columns*0.035)
...
def findparallel2(all_lines, columns):    
    ...
    if all_lines is not None:
        parallel_lines = []
        dist_threshold = columns*0.25
        biggest_line_index, biggest_line_angle = find_biggest_line(all_lines=all_lines)
        l = len(all_lines)
        if l==1:
...
if __name__== '__main__':
    ...
    img_bilat2 = cv.bilateralFilter(result_prep, d=30, sigmaColor=100, sigmaSpace=50)
    # img_meanshift = cv.pyrMeanShiftFiltering(img_bilat, sp=20, sr=20)
    # img_meanshift = cv.pyrMeanShiftFiltering(img_bilat2, sp=42, sr=24, maxLevel=4)
        # img_rgb = result_prep
        # img_hsv = cv.cvtColor(img_rgb, cv.COLOR_RGB2HSV)


        ##We apply Sobel to each channel
    def rest_of_pipeline(img_bilat):
        img_meanshift = cv.pyrMeanShiftFiltering(img_bilat, sp=42, sr=15, maxLevel=4)
        ...
        parallel_lines = findparallel2(all_lines, columns)
        # print(g_lines)
        # print(all_lines)
        # print(parallel_lines)
        ...
        return img_final, printed_lines_img, img_meanshift, r_edges_erode, g_edges_erode, b_edges_erode
    
    res1, pl1, mean1, re1, ge1, be1 = rest_of_pipeline(img_bilat)
    res2, pl2, mean2, re2, ge2, be2 = rest_of_pipeline(img_bilat2)
    ...
    # imshow('post meanshift 2', img_meanshift2)  
    ...
    cv.imwrite(save_path, img=img_res)
