...
def get_rgbchannel_lines(r_edges_erode, g_edges_erode, b_edges_erode, columns):
    ...
    min_gap = int(columns*0.040)
    r_lines = cv.HoughLinesP(r_edges_erode, 1, np.pi/180, 70, 50, min_long, min_gap)
    g_lines = cv.HoughLinesP(g_edges_erode, 1, np.pi/180, 70, 50, min_long, min_gap)
    b_lines = cv.HoughLinesP(b_edges_erode, 1, np.pi/180, 70, 50, min_long, min_gap)
...
if __name__== '__main__':
    ...
    img_bilat = cv.bilateralFilter(result_prep, d=30, sigmaColor=80, sigmaSpace=10)
   
    # img_meanshift = cv.pyrMeanShiftFiltering(img_bilat, sp=45, sr=25, maxLevel=3)
    


    
    
    mask = np.zeros(img_bilat.shape[:2],np.uint8)
    bgdModel = np.zeros((1,65),np.float64)
    fgdModel = np.zeros((1,65),np.float64)
    rect = (1,1,rows,columns)
    cv.grabCut(img_bilat,mask,rect,bgdModel,fgdModel,10,cv.GC_INIT_WITH_RECT)
    mask2 = np.where((mask==2)|(mask==0),0,1).astype('uint8')
    img_bilat = img_bilat*mask2[:,:,np.newaxis]
    # plt.imshow(img_meanshift),plt.colorbar(),plt.show()
    ...
    printed_lines_img = img_bilat.copy()
    ...
    imshow('post meanshift', printed_lines_img)  
